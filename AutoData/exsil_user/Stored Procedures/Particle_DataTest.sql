...
CREATE  PROCEDURE exsil_user.Particle_DataTest
	(
...
insert @Data
	SELECT     substring(Tencor_Data.[ID#],1,4)
		, Tencor_Data.[RUN#]
		, Tencor_Data.WAFER_LOG
		, Tencor_Data.RECIPE
		, Tencor_Data.SORT
		, Tencor_Data.[500_BIN]
		, Tencor_Data.[1000_BIN]
		, Tencor_Data.[130_BIN]
		, Tencor_Data.[160_BIN]
		, Tencor_Data.[200_BIN]
		, Tencor_Data.[250_BIN]
		, Tencor_Data.[300_BIN]
		, convert(nVarChar(10)
		, cast(Tencor_Data.[DATE] as smalldatetime),101)
	FROM         dbo.Tencor_Data INNER JOIN
						@Lots A ON RIGHT(A.Lot, 4) = Tencor_Data.WAFER_LOG 
							AND SUBSTRING(A.Lot, 6, LEN(A.Lot) - 10) = Tencor_Data.[RUN#]
	WHERE @MainID=(case when Tencor_Data.SORT = 'PASS'then substring(Tencor_Data.RECIPE,charindex('-',Tencor_Data.RECIPE,1)+1,4) else Tencor_Data.SORT end)
insert @Data
	SELECT substring(CR81_DATA.[ID#],1,4)
		, CR81_DATA.[RUN#]
		, CR81_DATA.WAFER_LOG
		, CR81_DATA.RECIPE
		, CR81_DATA.GRADE
		...
		, CR81_DATA.[250_BIN]
		, CR81_DATA.[300_BIN]
		, convert(nVarChar(10),cast(CR81_DATA.[DATE] as smalldatetime),101)
	FROM dbo.CR81_DATA INNER JOIN
			@Lots A ON RIGHT(A.Lot, 4) = CR81_DATA.WAFER_LOG 
				AND SUBSTRING(A.Lot, 6, LEN(A.Lot) - 10) = CR81_DATA.[RUN#]
	WHERE @MainID=(case when ltrim(CR81_DATA.GRADE)='PASS' then substring(CR81_DATA.RECIPE,3,4) else ltrim(CR81_DATA.GRADE) end)
		
if @SODs=1 begin
insert @Data
	SELECT     substring(SP1_Data.[ID#],1,4), SP1_Data.[RUN#], SP1_Data.Wafer_log, SP1_Data.SPRecipeName, SP1_Data.DispositionName, 
	...
	FROM         dbo.SP1_Data INNER JOIN
		...
		= SP1_Data.[RUN#]
	WHERE     @MainID=(case when substring(SP1_Data.DispositionName,1,4) ='PASS' then substring(SP1_Data.SPRecipeName,4,4) else SP1_Data.DispositionName end)
...
insert @Data
SELECT substring(SP1_Data.[ID#],1,4)
	,SP1_Data.[RUN#]
	,SP1_Data.Wafer_log
...
if @Cnt<@SampleSize begin
	insert @Data
		SELECT substring(CR81_DATA.[ID#],1,4)
			...
			,CR81_DATA.[300_BIN]
			,convert(nVarChar(10)
			,cast(CR81_DATA.[DATE] as smalldatetime),101)
		FROM dbo.CR81_DATA INNER JOIN
				@Lots A ON RIGHT(A.Lot, 4) = CR81_DATA.WAFER_LOG 
					AND SUBSTRING(A.Lot, 6, LEN(A.Lot) - 10) = CR81_DATA.[RUN#]
		WHERE (case when ltrim(CR81_DATA.GRADE)='PASS' then substring(CR81_DATA.RECIPE,3,4) else ltrim(CR81_DATA.GRADE) end) 
	...
	insert @Data
		...
		FROM         dbo.SP1_Data INNER JOIN
			...
			@Lots A ON RIGHT(A.Lot, 4) = SP1_Data.Wafer_log AND SUBSTRING(A.Lot, 6, LEN(A.Lot) - 10) 
			= SP1_Data.[RUN#]
		WHERE (case when substring(SP1_Data.DispositionName,1,4) ='PASS' then substring(SP1_Data.SPRecipeName,4,4) else SP1_Data.DispositionName end) 
				in (SELECT [From] FROM [$(ALTS)].dbo.TransferID_ByStage WHERE   (StageName = N'Final Pack' or StageName = N'PWC' or StageName = N'ALL') AND ([To] = @MainID))
			AND (SP1_Data.SOD2 < 738) 
			AND (SP1_Data.SOD4 < 200) 
			--AND (SP1_Data.BinCnt4 < 200) 
